  ...
  ds.forEach((item, index) => {
    html += `
        <tr>
            ...
            <td>
                <a href="javaScript:void(0)" data-toggle="modal" data-target="#modelId" title='Sửa Điện thoại' onclick="updateMobile('${
                  item.Ma_so
                }')">
            ...
            <td>
                <a href="javaScript:void(0)" onclick="deleteMobile('${
                  item.Ma_so
    ...
    <div class="form-group">
        ...
        <select id="Th_Nhom_Dien_thoai">
            <option value="ANDROID" ${
              Nhom == "ANDROID" ? "selected" : ""
            } >ANDROID</option>
            <option value="IPHONE" ${
              Nhom == "IPHONE" ? "selected" : ""
            }>IPHONE</option>
        </select>
    </div>
    <div class="form-group">
...
const autoKey = () => {
  ...
  arrNhom.sort((a, b) => {
    return (
      Number(a.Ma_so.trim().split("_")[1]) -
      Number(b.Ma_so.trim().split("_")[1])
    );
  });
...
const taoBaocaoMobile = () => {
  ...
  Highcharts.chart("Th_Bieu_do_Mobile", {
    ...
    subtitle: {
      text: " Thống kê Điện thoại theo Nhà Sản xuất",
    },
    xAxis: {
    ...
    caption: {
      text: `Thống kê Số lượng Nhập hàng Điện thoại Theo Nhà Sản xuất `,
    },
